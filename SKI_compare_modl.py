...
def write_Data(psnr,ssim,hfen):
    with open(osp.join('./result/compare_modl/SKI/',"psnr_compare_modl.txt"),"w+") as f:
        f.writelines('['+str(round(psnr, 3))+' '+str(round(ssim, 4))+' '+str(round(hfen, 3))+']')
        f.write('\n')

def write_Data2(step,psnr,ssim,hfen):
    with open(osp.join('./result/compare_modl/SKI/',"psnr_T.txt"),"w+") as f:
...
#============================================================================================
    ...
    write_images(abs(ori_complex),osp.join('./result/compare_modl/SKI/'+'ori'+'.png'))
    io.savemat(osp.join('./result/compare_modl/SKI/'+'ori'),{'img':ori_complex})
    ...
    write_images(abs(zero_fiiled),osp.join('./result/compare_modl/SKI/'+'zero_fiiled'+'.png'))
    io.savemat(osp.join('./result/compare_modl/SKI/'+'zero_fiiled'),{'img':zero_fiiled})
    ...
    for i in range(FLAGS.num_steps):
        ...
        x_mod_I[:,:,:,0] = np.real(I_temp)
        x_mod_I[:,:,:,1] = np.imag(I_temp)
        ...
        # Data Consistance 
        iterkspace = undersample_kspace + FT(I_complex,csm)*(1-mask)
        ...
        if write_psnr<psnr:
            ...
            write_images(abs(im_complex),osp.join('./result/compare_modl/SKI/'+'SKI_EBMrec_'+'.png'))
            write_images(abs(err)*5,osp.join('./result/compare_modl/SKI/'+'erro_CompareMmodl'+'.png'))
            io.savemat(osp.join('./result/compare_modl/SKI/'+'SKI_EBM_rec'),{'img':im_complex})
